...
$(".cabeceraHabitacion ul.nav li.nav-item a").click(function(e){
    ...
    /*e.preventDefault() evita que el href se active*/
	e.preventDefault();
	...
	$.ajax({
		...
		success:function(respuesta){
			...
			for(var i = 0; i < galeria.length; i++){
				...
				$(listaSlide[0]).html('<img class="img-fluid" src="'+galeria[galeria.length-1]+'">')

				$(listaSlide[i+1]).html('<img class="img-fluid" src="'+galeria[i]+'">')

				$(listaSlide[galeria.length+1]).html('<img class="img-fluid" src="'+galeria[0]+'">')
			}
            ...
            //
			 $('input[name="id-habitacion"]').val(respuesta[orden]["id"]);
